...
def get_db_connection():
    try:
        ...
        error_logger.error(f"Error connecting to MySQL: {e}")
...
async def record_user_event(event_data: dict = Body(...)):
    """Endpoint to record a user event."""
    try:
        ...
        # Hashing the user_id for anonymization
        ...
        VALUES (%s, %s, %s)
        """
        execute_query(query, (hashed_user_id, event_type, timestamp), fetch=False)

        return {"status": "success", "detail": "Event recorded"}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

@router.get("/analytics/stats")
async def get_analytics_stats():
    """Endpoint to get analytics statistics."""
    try:
        # Query to get the count of events per user
        query = """
        ...
        stats = [{"user_id": r["user_id"], "event_count": r["event_count"]} for r in results]
...
def record_user_event(user_id: str, session_id: str, event_type: str, event_data: Dict = None):
    if not user_id:
        logger.warning("No user_id provided for analytics event")
        return
    ...
    connection = None
    try:
        logger.info(f"Recording analytics event: {event_type} for user {user_id} session {session_id}")
        connection = get_db_connection()
        ...
        if event_type == "session_start":
            logger.info(f"Recording session start for user {user_id}")
            ...
            logger.info(f"Recording question for user {user_id}: {event_data.get('question', '')[:50]}...")
            ...
            if conversation_id:
                ...
                logger.info(f"Inserted user message: {message_id}")
            ...
            logger.info(f"Recording bot response for user {user_id}")
            ...
            if conv:
                ...
                logger.info(f"Inserted bot message: {message_id}")
            else:
                logger.warning(f"No active conversation found for session {session_id}")

        elif event_type == "session_end":
            logger.info(f"Recording session end for user {user_id}")
            ...
            logger.info(f"Recording user left for user {user_id}")
        ...
        if connection:
            connection.commit()
            logger.info(f"Successfully committed analytics event: {event_type}")

    except Error as e:
        error_logger.error(f"Error recording user event {event_type}: {e}")
        if connection:
            connection.rollback()
            error_logger.error("Analytics transaction rolled back")
    except Exception as e:
        error_logger.error(f"Unexpected error recording user event {event_type}: {e}")
        if connection:
            connection.rollback()
            error_logger.error("Analytics transaction rolled back")
    finally:
...
async def get_session_analytics():
    try:
        ...
        error_logger.error(f"Error in session analytics: {str(e)}")
        return {
            "active_sessions": 0,
            "today_sessions": 0,
            "average_duration": 0,
...
async def get_conversation_analytics():
    try:
        ...
        error_logger.error(f"Error in conversation analytics: {str(e)}")
        return {
...
async def get_message_analytics():
    try:
        ...
        error_logger.error(f"Error in message analytics: {str(e)}")
        return {
            "total_messages": 0,
            "user_messages": 0,
            "bot_messages": 0,
...
async def get_user_analytics_by_id(user_id: str):
    try:
        ...
        error_logger.error(f"Error in user analytics: {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def capture_lead(lead_data: dict):
    try:
        ...
        error_logger.error(f"Error capturing lead: {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_lead_analytics():
    try:
        ...
        error_logger.error(f"Error in lead analytics: {str(e)}")
        return {
            "total_leads": 0,
            "daily_leads": []
        }
...
async def record_human_handover(data: dict = Body(...)):
    try:
        logger.info(f"Received handover data: {data}")
        ...
        if requested_at:
            try:
                ...
                error_logger.error(f"Error parsing requested_at: {e}")
        ...
        error_logger.error(f"Error recording human handover: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_human_handover_analytics():
    try:
        ...
        error_logger.error(f"Error in human handover analytics: {e}")
    ...
    try:
        ...
        if closed_at:
            try:
                ...
                error_logger.error(f"Error parsing closed_at: {e}")
        ...
        error_logger.error(f"Error recording chatbot close: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def record_session_end(data: dict = Body(...)):
    try:
        ...
        error_logger.error(f"Error recording session end: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def record_user_left(data: dict = Body(...)):
    ...
    try:
        ...
        error_logger.error(f"Error recording user left: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def mark_inactive_users_timeout():
    ...
    try:
        ...
        error_logger.error(f"Error marking users inactive due to timeout: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
def get_events():
    try:
        ...
        error_logger.error(f"Error in get_events: {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))